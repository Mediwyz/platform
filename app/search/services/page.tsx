      ...
      <div className="bg-surface border-b border-line sticky top-0 z-30">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-5">
          <div className="flex items-center gap-3 mb-3">
            <Link href="/" className="text-sm text-soft hover:text-soft">Home</Link>
            <span className="text-faint">/</span>
            <span className="text-sm font-medium text-fg">Services</span>
          </div>

          <div className="flex flex-col sm:flex-row sm:items-center gap-3">
            <div className="flex items-center gap-2">
              <FaConciergeBell className="text-[#0C6780] text-xl" />
              <h1 className="text-xl sm:text-2xl font-bold text-fg">All Services</h1>
              {!loading && (
                <span className="text-sm text-faint font-normal">({filtered.length})</span>
              )}
            </div>
              ...
              <button
                ...
                className={`flex-shrink-0 px-3 py-1 rounded-full text-[11px] font-medium transition-all border
                  ${activeCategory === 'ALL'
                    ...
                    : 'bg-surface text-soft border-line hover:border-gray-400'}`}
              >
                All Categories
              </button>
        ...
        {loading ? (
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
            {Array.from({ length: 15 }).map((_, i) => (
              <div key={i} className="animate-pulse bg-surface rounded-2xl h-44 border border-line" />
            ))}
          </div>
        ) : filtered.length === 0 ? (
          <div className="text-center py-24">
            <FaConciergeBell className="text-5xl text-gray-200 mx-auto mb-4" />
            <p className="text-base font-semibold text-soft">No services found</p>
            ...
            {filtered.map(svc => {
              ...
              return (
                <Link
                  ...
                  className="group flex flex-col bg-surface rounded-2xl border border-line
                ...
                >
                  <div className="p-3 sm:p-4 flex-1 flex flex-col">
                    ...
                    {svc.description && (
                      <p className="text-[11px] text-soft leading-relaxed line-clamp-2 flex-1">
                        {svc.description}
                      </p>
                    )}
                  </div>

                  <div className="px-3 sm:px-4 pb-3 pt-2 border-t border-line">
