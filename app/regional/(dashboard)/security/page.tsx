...
export default function SuperAdminSecurityPage() {
 ...
 <h2 className="text-lg font-semibold text-fg mb-4">Security Settings</h2>
 ...
 <div>
 <p className="text-sm font-medium text-fg">{setting.label}</p>
 <p className="text-xs text-soft">{setting.desc}</p>
 </div>
 <div className={`w-10 h-6 rounded-full ${setting.enabled ? 'bg-green-500' : 'bg-gray-300'} relative`}>
 <div className={`w-4 h-4 bg-surface rounded-full absolute top-1 transition-all ${setting.enabled ? 'right-1' : 'left-1'}`} />
 </div>
 </div>
 ))}
 </div>
 ...
 <div className="bg-surface rounded-xl p-6 shadow-lg">
 <h2 className="text-lg font-semibold text-fg mb-4">Recent Security Events</h2>
 {loading ? (
 ...
 <FaShieldAlt className="text-4xl mx-auto mb-3 text-faint" />
 ...
 {events.map((event) => (
 <div key={event.id} className="flex items-start gap-3 p-3 bg-subtle rounded-lg">
 ...
 <span className="text-xs text-soft">{event.ip}</span>
 <span className="text-xs text-soft">{new Date(event.timestamp).toLocaleString()}</span>
 </div>
 </div>
 </div>
 ))}
