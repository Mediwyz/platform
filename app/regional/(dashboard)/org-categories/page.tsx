...
export default function OrgCategoriesPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      {loading ? (
        <div className="flex justify-center py-16 text-faint"><FaSpinner className="animate-spin text-2xl" /></div>
      ) : (
        <div className="bg-surface rounded-xl border border-line divide-y divide-line overflow-hidden">
          ...
          {cats.length === 0 && <p className="text-center text-faint py-10 text-sm">No categories yet.</p>}
        </div>
      )}

      {showModal && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <form onSubmit={submit} className="bg-surface rounded-2xl w-full max-w-md p-6 shadow-xl">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-bold text-fg">{editingId ? 'Edit category' : 'New category'}</h2>
              <button type="button" onClick={() => setShowModal(false)} className="text-faint hover:text-soft"><FaTimes /></button>
            </div>
            {error && <p className="text-sm text-red-500 mb-3">{error}</p>}
            <div className="space-y-3">
              <div>
                <label className="block text-xs font-medium text-soft mb-1">Label</label>
                <input value={form.label} onChange={e => setForm({ ...form, label: e.target.value })} required className="w-full border border-line rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-[#0C6780] outline-none" placeholder="e.g. Pharmacy" />
              </div>
              {!editingId && (
                <div>
                  <label className="block text-xs font-medium text-soft mb-1">Key (auto from label if empty)</label>
                  <input value={form.key} onChange={e => setForm({ ...form, key: e.target.value })} className="w-full border border-line rounded-lg px-3 py-2 text-sm font-mono focus:ring-2 focus:ring-[#0C6780] outline-none" placeholder="pharmacy" />
                </div>
              )}
              <div>
                <label className="block text-xs font-medium text-soft mb-1">Description</label>
                <input value={form.blurb} onChange={e => setForm({ ...form, blurb: e.target.value })} className="w-full border border-line rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-[#0C6780] outline-none" placeholder="Pharmacies & drugstores" />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-medium text-soft mb-1">Icon (react-icons/fa)</label>
                  <input value={form.icon} onChange={e => setForm({ ...form, icon: e.target.value })} className="w-full border border-line rounded-lg px-3 py-2 text-sm font-mono focus:ring-2 focus:ring-[#0C6780] outline-none" placeholder="FaHospital" />
                </div>
                <div>
                  <label className="block text-xs font-medium text-soft mb-1">Order</label>
                  <input type="number" value={form.displayOrder} onChange={e => setForm({ ...form, displayOrder: parseInt(e.target.value) || 100 })} className="w-full border border-line rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-[#0C6780] outline-none" />
                </div>
              </div>
              <label className="flex items-center gap-2 text-sm text-soft">
                <input type="checkbox" checked={form.isActive} onChange={e => setForm({ ...form, isActive: e.target.checked })} className="rounded border-line text-[#0C6780] focus:ring-[#0C6780]" />
                Active
              </label>
            </div>
            <div className="flex gap-3 mt-6">
              <button type="button" onClick={() => setShowModal(false)} className="flex-1 py-2.5 border border-line rounded-lg text-sm font-medium text-soft hover:bg-subtle">Cancel</button>
