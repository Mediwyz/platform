...
const AdminManagementPage = () => {
 ...
 <p className="text-soft">Approve, manage, and monitor all regional admin accounts.</p>
 </div>
 <Link href="/regional" className="px-4 py-2 border rounded-lg hover:bg-subtle">
 Back to Dashboard
 </Link>
 </div>
 </header>
 ...
 {/* Filters */}
 <div className="bg-surface p-4 rounded-xl shadow mb-6 flex justify-between items-center flex-wrap gap-4">
 ...
 filter === f.key ? 'bg-blue-600 text-white' : 'hover:bg-subtle text-soft'
 }`}
 >
 <f.icon className="text-base" />
 <span>{f.label}</span>
 <span className={`text-xs px-1.5 py-0.5 rounded-full ${
 filter === f.key ? 'bg-blue-500 text-white' : 'bg-line text-soft'
 }`}>
 ...
 <div key={admin.id} className="bg-surface rounded-xl shadow p-4 flex items-center justify-between flex-wrap gap-4">
 ...
 />
 <div>
 <p className="font-semibold text-fg">{admin.name}</p>
 <p className="text-sm text-soft">{admin.email}</p>
 <p className="text-xs text-soft">{admin.role}</p>
 </div>
 </div>
 ...
 <p className="text-sm text-soft">Joined: {admin.joinDate}</p>
