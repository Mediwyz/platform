            ...
            <Link
              ...
              className={`flex items-center gap-3 p-3 rounded-lg border transition-colors ${
                isDone
                  ...
                  : 'bg-surface border-line hover:border-[#0C6780]'
              }`}
            >
              <div
                className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${
                  isDone ? 'bg-green-500 text-white' : 'bg-subtle text-faint border border-line'
                }`}
              >
                {isDone ? <FaCheck className="text-xs" /> : <span className="text-xs font-bold">{steps.indexOf(step) + 1}</span>}
              </div>
              <div className="flex-1 min-w-0">
                <p className={`font-medium text-sm ${isDone ? 'text-green-700 line-through' : 'text-fg'}`}>
                  {step.label}
                </p>
                <p className="text-xs text-soft">{step.description}</p>
              </div>
              {!isDone && <FaChevronRight className="text-faint text-xs" />}
            </Link>
          )
        })}
      </div>
