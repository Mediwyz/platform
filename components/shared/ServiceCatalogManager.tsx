...
export default function ServiceCatalogManager({ config }: { config: ServiceCatalogConfig }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            className={`w-full pl-10 pr-4 py-2.5 border border-line rounded-lg focus:outline-none focus:border-${accent}-500 focus:ring-2 focus:ring-${accent}-200 transition text-sm`}
          />
        </div>
        <div className="relative">
          ...
          <select
            ...
            className="pl-10 pr-8 py-2.5 border border-line rounded-lg focus:outline-none focus:border-teal-500 text-sm appearance-none bg-surface"
          >
      ...
      {/* Service Cards */}
      {filtered.length === 0 ? (
        <div className="bg-surface rounded-xl p-12 text-center shadow-sm border border-line">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map(service => {
            ...
            return (
              <div key={service.id} className="bg-surface rounded-xl shadow-sm border border-line hover:shadow-md transition-shadow overflow-hidden">
                {/* Illustration strip */}
                ...
                <div className="p-5">
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex-1 min-w-0">
                      <h3 className="font-semibold text-fg truncate">{service.serviceName}</h3>
                    ...
                    <div className="flex items-center gap-1 ml-2">
                      <button
                        ...
                        className="p-1.5 text-faint hover:text-blue-600 hover:bg-blue-50 rounded-lg transition"
                        title="Edit"
                      >
                      ...
                      <button
                        ...
                        className="p-1.5 text-faint hover:text-red-600 hover:bg-red-50 rounded-lg transition"
                        title="Delete"
                      >
                  ...
                  <p className="text-sm text-soft mb-3 line-clamp-2">{service.description}</p>
                  <div className="flex items-center justify-between pt-3 border-t border-line">
                    <span className="text-lg font-bold text-fg">
                      {service.currency || 'MUR'} {((service.defaultPrice as number) ?? service.price ?? 0).toLocaleString()}
                    </span>
                    <span className={`text-xs px-2 py-0.5 rounded-full ${service.isActive ? 'bg-green-50 text-green-700' : 'bg-subtle text-soft'}`}>
                  ...
                  {config.providerType && (
                    <div className="pt-3 border-t border-line mt-3">
                      <p className="text-xs font-medium text-soft mb-1">Linked Workflow</p>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-surface rounded-2xl w-full max-w-lg max-h-[92vh] overflow-y-auto">
            <div className="flex items-center justify-between p-5 border-b sticky top-0 bg-surface z-10">
              <h2 className="text-lg font-bold text-fg">
                {editingService ? 'Edit Service' : 'Add New Service'}
              </h2>
              <button onClick={() => setShowModal(false)} className="p-2 hover:bg-subtle rounded-lg">
                <FaTimes />
              </button>
            </div>
            ...
            <div className="p-5 space-y-4">
              ...
              <div>
                ...
                {/* Tab toggle */}
                <div className="flex gap-1 mb-3 bg-subtle p-1 rounded-lg">
                  <button
                    type="button"
                    ...
                    className={`flex-1 flex items-center justify-center gap-1.5 py-1.5 rounded-md text-xs font-medium transition
                      ${illustrationTab === 'icon' ? 'bg-surface text-fg shadow-sm' : 'text-soft hover:text-soft'}`}
                  >
                  ...
                  <button
                    ...
                    className={`flex-1 flex items-center justify-center gap-1.5 py-1.5 rounded-md text-xs font-medium transition
                      ${illustrationTab === 'upload' ? 'bg-surface text-fg shadow-sm' : 'text-soft hover:text-soft'}`}
                  >
                    ...
                    <div className="flex items-center gap-3 mb-2">
                      <div className="w-14 h-14 rounded-xl bg-teal-50 flex items-center justify-center flex-shrink-0 border border-teal-100">
                        {currentEmoji ? (
                          ...
                          <FaIcons className="text-2xl text-faint" />
                        )}
                      </div>
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex gap-2">
                          ...
                          {(currentIconKey || currentEmoji) && (
                            <button
                              ...
                              className="text-xs px-2.5 py-1 bg-subtle text-soft rounded-lg hover:bg-line transition"
                            >
                              Clear
                            </button>
                          )}
                  ...
                  <div>
                    ...
                    {currentImageUrl ? (
                      <div className="flex items-center gap-3">
                        <Image src={currentImageUrl} alt="Service illustration" width={96} height={64} className="w-24 h-16 object-contain rounded-lg border border-line bg-subtle" />
                      ...
                      <button
                        ...
                        className="w-full flex flex-col items-center justify-center gap-2 py-6 border-2 border-dashed border-line rounded-xl text-faint hover:border-[#0C6780] hover:text-[#0C6780] transition-colors disabled:opacity-50"
                      >
                        {uploadingImage ? (
                          <>
                    ...
                    <p className="text-[10px] text-faint mt-1.5">Upload a custom illustration for this service. Use a square or landscape image for best results.</p>
                  </div>
                )}
              </div>
                ...
                <div key={field.key}>
                  ...
                  {field.type === 'select' ? (
                    <select
                      ...
                      className="w-full px-3 py-2.5 border border-line rounded-lg focus:outline-none focus:border-teal-500 text-sm"
                    >
                    ...
                    <textarea
                      ...
                      rows={3}
                      className="w-full px-3 py-2.5 border border-line rounded-lg focus:outline-none focus:border-teal-500 text-sm resize-none"
                    />
                  ) : field.type === 'checkbox' ? (
                    <label className="flex items-center gap-2">
                      ...
                      />
                      <span className="text-sm text-soft">Enabled</span>
                    </label>
                  ) : (
                    <input
                      type={field.type}
              ...
              {config.providerType && (
                <div>
                  ...
                  {editingService ? (
                    <div className="rounded-lg border border-line bg-subtle px-3 py-2.5 text-sm text-soft">
                    ...
                    <div className="space-y-2">
                      <select
                        ...
                        className={`w-full px-3 py-2.5 border rounded-lg focus:outline-none focus:border-teal-500 text-sm ${!selectedWorkflowId ? 'border-amber-300 bg-amber-50' : 'border-line'}`}
                      >
                      ...
                      {selectedWorkflowId && (() => {
                        ...
                        if (!wf) return null
                        return (
                          <div className={`inline-flex items-center gap-1.5 px-2 py-1 rounded-full text-xs font-medium ${MODE_COLORS[wf.serviceMode] || 'bg-subtle text-soft'}`}>
            ...
            <div className="flex items-center justify-end gap-3 p-5 border-t sticky bottom-0 bg-surface">
              <button
                onClick={() => setShowModal(false)}
                className="px-4 py-2.5 text-soft bg-subtle rounded-lg hover:bg-line transition text-sm font-medium"
              >
                Cancel
              </button>
              <button
