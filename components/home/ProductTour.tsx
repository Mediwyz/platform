            ...
            {TABS.map(t => (
              <button
                ...
                className={`px-5 sm:px-6 py-2 rounded-full text-sm font-semibold transition-colors ${
                  tab === t.id ? 'bg-white text-[#0C6780] shadow-sm' : 'text-gray-500 hover:text-gray-700'
                }`}
              >
                {t.label}
              </button>
        ...
        <div className="grid lg:grid-cols-[1.6fr_1fr] gap-6 lg:gap-8 items-start">
          ...
          <div className="rounded-2xl border border-gray-200 shadow-xl overflow-hidden bg-white">
            <div className="flex items-center gap-1.5 px-4 py-2.5 bg-gray-50 border-b border-gray-100">
              ...
              <span className="ml-3 text-xs text-gray-400 truncate">mediwyz.com — {shot.title}</span>
            </div>
            <div className="relative aspect-[1280/820] bg-gray-50">
              <Image
                key={shot.key}
